...
# a = list(map(lambda x: [x.count(i) for i in glas if i in x], text))

# b = sum(a, [])

# if (len(set(b)) == 1) : print ('Парам пам-пам')
...
# lst = [[1] * rows for i in range(columns)]

# for i in range(columns):
#   for j in range(rows):
#     lst[i][j] = (i + 1) * (j + 1)
# print(tabulate(list(zip(*lst))))